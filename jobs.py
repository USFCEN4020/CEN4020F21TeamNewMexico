...
def list_jobs_saved(username):
    ...
    tmpcon = db.sqlite3.connect('inCollege.db')
    tmpcursor = tmpcon.cursor()
    jobs = tmpcursor.execute("SELECT * FROM app_status WHERE username = '{}' AND status = 'saved' COLLATE NOCASE".format(username)).fetchall()
    store_list = []
    count = 1
    print("0. Go Back")
    for i in jobs:
...
def list_jobs_applied(username):
    ...
    tmpcon = db.sqlite3.connect('inCollege.db')
    tmpcursor = tmpcon.cursor()
    jobs = tmpcursor.execute("SELECT * FROM app_status WHERE username = '{}' AND status = 'applied' COLLATE NOCASE".format(username)).fetchall()
    store_list = []
    count = 1
    print("0. Go Back")
    for i in jobs:
...
def apply_job(job, current_user):
    ...
    if status == "applied":
        ...
        tmpcursor.execute("UPDATE app_status SET status = 'applied' WHERE username = '{}' AND title = '{}' AND status = 'saved'".format(current_user, job[1]))
    #the none category means hasnt applied yet or saved yet
    else:
        tmpcursor.execute("INSERT INTO app_status VALUES ('{}', '{}', '{}', 'applied')".format(current_user, job[1], job[0]))
    ...
    tmpcursor.execute("INSERT INTO applications VALUES ('{}', '{}', '{}', '{}', '{}', '{}')".format(current_user, job[1], job[3], grad_d, start_d, parag))
    tmpcon.commit()
